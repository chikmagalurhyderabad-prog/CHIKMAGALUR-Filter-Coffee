...
export default function Products({
  ...
  return (
    <section className="py-24 bg-[#FAF7F2] border-t border-[#3D2B1F]/10">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-16">
          <span className="text-[#B48C44] text-xs uppercase tracking-[0.3em] font-sans font-bold block mb-4">
            {tag}
          </span>
          <h2 className="text-4xl lg:text-5xl font-medium text-[#3D2B1F] tracking-tight">
            {title}
          </h2>
          <p className="mt-4 text-lg opacity-80 text-[#3D2B1F]">
            {subtitle}
          </p>
        </div>
        ...
        {isLoading ? (
          ...
          <div className="text-center py-16 border border-[#3D2B1F]/10 bg-[#FAF7F2]">
            <p className="text-lg text-[#3D2B1F] opacity-70 italic font-serif">No products found in this selection.</p>
          </div>
        ) : (
        ...
        {showViewAll && (
          <div className="mt-16 text-center">
            <Link
              to="/shop"
              className="inline-block border border-[#3D2B1F] text-[#3D2B1F] px-10 py-5 text-[11px] uppercase tracking-[0.2em] font-sans font-bold hover:bg-[#3D2B1F] hover:text-[#FAF7F2] transition-colors"
            >
              VIEW ALL PRODUCTS
            </Link>
          </div>
